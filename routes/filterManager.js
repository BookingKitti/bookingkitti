...
exports.search_admin_hotel_info=function(hotel_name, province, city, addr, date_in, date_out, l_price, h_price, callback) {
  var sql = "select * from HotelInfo right outer join (select Hotel_ID, min(Price) \
  as Min_Price from RoomInfo group by Hotel_ID) as T right outer join (select Hotel_ID, \
  ...
  //if user did not input any filter condition, just delete "where" in sql sentence
  if (cond_list.length == 0)
      sql = "select * from HotelInfo right outer join (select Hotel_ID, min(Price) \
      as Min_Price from RoomInfo group by Hotel_ID) as T right outer join (select Hotel_ID, \
        min(File_Pos) as A_File_Pos from HotelPics group by Hotel_ID) as G ";
  else
      sql = sql + cond_list[0];
